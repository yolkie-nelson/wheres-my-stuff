...
const StorageSiteList = () => {
    ...
    const confirmDelete = async () => {
        const response = await deleteStorageSite(deletedStorageSite)
                ...
                <ul>
                    <div className="max-h-[42rem] overflow-y-scroll">
                        {storageSites?.map((storageSite) => {
                            const matchingEquipment = equipmentList?.filter(
                                (equipment) =>
                                    equipment.storage_site_id === storageSite.id
                            )
                            return (
                                <li key={storageSite.id} className="mb-4">
                                    <div className="border border-gray-300 bg-white rounded-md p-4">
                                        <h2 className="text-xl font-semibold mb-2">
                                            Warehouse Number:{' '}
                                            {storageSite.warehouse_number}
                                        </h2>
                                        <p className="text-gray-600 py-4">
                                            Address:{' '}
                                            {storageSite.location_address}
                                        </p>
                                        <p className="text-gray-600 pb-4">
                                            Point of Contact:{' '}
                                            {storageSite.point_of_contact}
                                        </p>
                                        <button
                                            onClick={() =>
                                                handleDelete(storageSite.id)
                                            }
